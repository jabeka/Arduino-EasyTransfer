...
void EasyTransfer::begin(uint8_t *ptr, uint16_t length, Stream *theStream)
{
  rx_len = 0;
  rx_array_inx = 0;
  address = ptr;
  ...
  rx_buffer = (uint8_t *)malloc(size + 4);
}

// Sends out struct in binary, with header, length info and checksum
void EasyTransfer::sendData()
{
  uint8_t CS = 0x00FF & size;
  CS ^= (0xFF00 & size) >> 8;
  _stream->write(0x06);
  _stream->write(0x85);
...
boolean EasyTransfer::receiveData()
{
  ...
  if (rx_len != 0)
  {
    ...
    if (rx_len == (rx_array_inx - 1))
    {
      ...
      // last uint8_t is CS
      calc_CS = rx_len & 0x00FF;
      calc_CS ^= ((rx_len & 0xFF00) >> 8);
